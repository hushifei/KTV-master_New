...
//
//  Created by stevenhu  on 15/11/26.
//  Copyright © 2015年 stevenhu.new. All rights reserved.
//
...
// 初始化滚动图片新特性界面
+ (instancetype)newFeatureVCWithImageNames:(NSArray *)imageNames enterBlock:(void(^)())enterBlock configuration:(void (^)(UIButton *enterButton,UIButton *demoButton))configurationBlock;
{
    ...
    //控制器准备
    configurationBlock([newFeatureVC vcPrepare],newFeatureVC.demoEnterButton);
...
- (UIButton *)imageViewsPrepare{
    ...
    [self.imageNames enumerateObjectsUsingBlock:^(NSString *imageName, NSUInteger idx, BOOL *stop) {
        ...
        if(idx == self.imageNames.count -1) {
            ...
            _demoEnterButton = [self setUpDemoEnterButton:imageV];
...
/** 初始化DEMO按钮 */
- (UIButton *)setUpDemoEnterButton:(UIImageView *)imageView
{
    imageView.userInteractionEnabled = YES;
    UIButton *enterDemoButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [imageView addSubview:enterDemoButton];
    [enterDemoButton addTarget:self action:@selector(chickDemoEnterButton) forControlEvents:UIControlEventTouchUpInside];
    return enterDemoButton;
}
...
- (void)chickDemoEnterButton
{
    
    [[DataManager instanceShare]unArchiveDemoDbFile:^(BOOL completed) {
