...
@implementation newSongListTopCell
...
- (void)layoutSubviews {
    ...
    _songName.frame=CGRectMake(CGRectGetMaxX(_numberStr.frame)+20, 8, 220, 20);
    ...
    if (IS_IPHONE_5) {
        ...
        _songName.frame=CGRectMake(CGRectGetMaxX(_numberStr.frame)+15, 8, 220, 20);
        ...
        if (self.buttonitem && self.oneSong.number.length > 0) {
            ...
            [cmd sendCmd_Diange:self.oneSong.number completed:^(BOOL completed, NSError *error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_numberStr shakeAndFlyAnimationToView:self.buttonitem];
                });
            }];
        }
    } else {
